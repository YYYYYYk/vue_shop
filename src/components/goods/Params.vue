          ...
          <el-table :data="manyTableData" border stripe>
            ...
            <el-table-column type="expand">
              ...
              <template slot-scope="scope">
                <el-button @click="ss(scope)">123</el-button>
                <!-- 循环渲染tag标签 -->
                <el-tag v-for="(item, i) in scope.row.attr_vals" :key="i" closable @close="handleClose(i, scope.row)">
                  {{item}}
                </el-tag>
                <!-- 显示tags输入文本框 -->
                <el-input  class="input-new-tag" v-if="scope.row.inputValue" v-model="scope.row.inputValue" ref="saveTagInput" @keyup.enter.native="handleInputConfirm(scope.row)" @blur="handleInputConfirm(scope.row)"></el-input>
                <!-- 添加tags标签按钮 -->
